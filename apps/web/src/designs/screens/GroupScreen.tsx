    ...
    <main className="dpe-page dpe-page--group">
      ...
      <div className="dpe-group-layout">
        ...
        <aside className="dpe-inspector">
          ...
          <section className="dpe-my-roles">
            <h3>我的角色与权限</h3>
            <ul className="dpe-my-roles__list">
              {(isOwner
                ? [{ roleId: "owner", name: "群主", color: "#9a6700", access_level: 3 }]
                : [
                ...
                <li key={r.roleId}>
                  <span className="dpe-my-roles__name" style={{ color: r.color }}>
                    {r.name}
                  </span>
            ...
            <p className="dpe-muted">
              在此节点的有效权限：
              <strong>{ROLE_LABELS[isOwner ? 3 : 2]}</strong>
            </p>
            {!isOwner && (
              <p className="dpe-muted" style={{ fontSize: 12, marginTop: 4 }}>
                多角色时取最高权限级别。
              </p>
            )}
          </section>
          ...
          {canManageAcl && (
            <details className="dpe-collapsible">
              <summary className="dpe-collapsible__summary">各角色权限</summary>
              <div className="dpe-collapsible__body">
                <p className="dpe-muted">配置每个群组角色在此节点上的权限。</p>
                <ul className="dpe-permission-list">
                  {roleAcls.map((r) => (
                    <li key={r.roleId}>
                      <span className="dpe-permission-list__role" style={{ color: r.color }}>
                        {r.name}
                      </span>
                      <select
